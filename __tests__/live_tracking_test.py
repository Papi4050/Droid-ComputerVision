...
class liveTrackingTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.img = live_tracking.face_recognition.load_image_file("./__tests"
                                                                 "__/resour"
                                                                 "ces/Luke."
                                                                 "jpeg")
        cls.faceCascade = live_tracking.cv2.CascadeClassifier("./__tests__/"
                                                              "resources/"
                                                              "haarcascade_"
                                                              "frontalface_"
                                                              "default.xml")
    ...
    def test_calculateTurnInput(self):
        ...
        res_right = live_tracking.calculateTurnInput(driveConfig,
                                                     1280, 10, "")
        res_left = live_tracking.calculateTurnInput(driveConfig,
                                                    1280, 1270, "")
        self.assertEqual(res_right, "$222027")
        self.assertEqual(res_left, "$333027")

    def test_findObjects(self):
        imgObjects, objectsOut = live_tracking.findObjects(self.img, self.
                                                           faceCascade)
